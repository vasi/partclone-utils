                ...
                if ((ofd = open(outfile, O_WRONLY | O_CREAT | O_LARGEFILE,
                    ...
                    if (freeblock) {
                        ...
                        if ((error = partclone_seek(pctx, freeblock)) == 0) {
                            if ((error = partclone_writeblocks(pctx, iobuf,
                                                               1)) == 0) {
                                ...
                                if (((error = partclone_open(
                                          argv[i], cfname, SYSDEP_OPEN_RW,
                                          &posix_dispatch, &pctx)) == 0) &&
                                    ((error = partclone_verify(pctx)) == 0) &&
                                    ((error = partclone_seek(
                                          pctx, freeblock)) == 0) &&
                                    ((error = partclone_readblocks(pctx, iobuf,
                                                                   1)) == 0)) {
