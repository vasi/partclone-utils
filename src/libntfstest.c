    ...
    for (i = 1; i < argc; i++) {
        ...
        if ((error = ntfsclone_open(argv[i], (char *)NULL, SYSDEP_OPEN_RW,
            ...
            if ((error = ntfsclone_verify(pctx)) == 0) {
                ...
                if ((ofd = open(outfile, O_WRONLY | O_CREAT | O_LARGEFILE,
                                0600)) >= 0) {
                        ...
                        if ((error = ntfsclone_seek(pctx, ci)) == 0) {
                            ...
                            if (blkstat == 1) {
                                if (lseek(ofd, ci * bsize, SEEK_SET) != -1) {
                                    if ((error = ntfsclone_readblocks(
                                             pctx, iobuf, 1)) == 0) {
                                        ssize_t nbout =
                                            write(ofd, iobuf, bsize);
                                        if (nbout != bsize) {
                                            error = (nbout < 0) ? errno : EIO;
                                            printf("%s: write error %d at "
                                                   "block %d\n",
                                                   outfile, error, ci);
                                            break;
                                        } else {
                    ...
                    if (freeblock) {
                        ...
                        if ((error = ntfsclone_seek(pctx, freeblock)) == 0) {
                            if ((error = ntfsclone_writeblocks(pctx, iobuf,
                                                               1)) == 0) {
                                printf("write test success! wrote \"%s\" to "
                                ...
                                if (((error = ntfsclone_open(
                                          argv[i], cfname, SYSDEP_OPEN_RW,
                                          &posix_dispatch, &pctx)) == 0) &&
                                    ((error = ntfsclone_verify(pctx)) == 0) &&
                                    ((error = ntfsclone_seek(
                                          pctx, freeblock)) == 0) &&
                                    ((error = ntfsclone_readblocks(pctx, iobuf,
                                                                   1)) == 0)) {
